...
"""Vendored RelGNN heterogeneous conv wrapper (``RelGNN_HeteroConv``).
...
STATUS: vendored from upstream; the only semantic change is the import.
  * Source: snap-stanford/RelGNN @ commit cffdb8b54627e92c7dd112c1243dde739c90d35b
    https://github.com/snap-stanford/RelGNN/blob/cffdb8b54627e92c7dd112c1243dde739c90d35b/examples/relgnn_hetero_conv.py
  * Modified: the ``RelGNNConv`` import is rewritten to the ``relarena.models.relgnn._vendor``
    package path; everything else is upstream (ruff normalizes formatting only).

Dispatches each atomic route to its :class:`RelGNNConv` and groups the per-route outputs
per destination node type, mirroring ``torch_geometric.nn.HeteroConv``.

Ruff-exempt (``# ruff: noqa``) so it stays a clean diff against upstream. To re-sync:
re-copy from the pinned file, redo the import rewrite, bump the commit above.
"""
