...
from relarena_core.metrics import (
    _METRICS,
    Metric,
...
def test_evaluate_predictions_preserves_target_and_native_metrics(
    include_primary: bool, held_out: bool
) -> None:
    native = Mock(__name__="native_metric")
    primary = Mock(__name__="primary_metric")
    metrics = [native, primary] if include_primary else [native]
    task = SimpleNamespace(
        metrics=metrics.copy(), evaluate=Mock(return_value={"score": 0.5})
    )
    target = None if held_out else object()
    predictions = np.array([0.2, 0.8])
    result = evaluate_predictions(task, predictions, target, primary)
    assert result == {"score": 0.5}
    task.evaluate.assert_called_once_with(
        predictions, target, metrics=[native, primary]
    )
